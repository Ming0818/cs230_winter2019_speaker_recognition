...
def generate_features(audio_entities, max_count, progress_bar=False):
    ...
    # Iterate over count_range, iteration number not needed
    # '_': throw away variable
    for _ in count_range:

        # Generates random sample from audio_entities
...
def normalize(list_matrices, mean, std):
    ...
    args:
        list_matrices(list): List of input matrices.
        ## Podria ser matricial
        mean(float): Mean value for list of matrices.
        std(float): Standard deviation value for list of matrices.
    return:
        n_list_matrices(list): List of normalized matrices

    '''
    n_list_matrices = [(m - mean) / std for m in list_matrices]
    return n_list_matrices
...
class InputsGenerator:
    ...
    def generate_inputs_for_inference(self, speaker_id):
        ...
        args:
            self(obj): InputsGenerator
            speaker_id(str): Speaker identification string. ('p' + speaker_number)
        return:
            feat(list): normalized feature...
    ...
    def generate_inputs(self, speaker_id):
        ...
        cutoff = int(len(audio_entities) * 0.95)
